...
//           }
	...
	*/
	...
	pba->tuning_index_2_smg = 2;
      ...
      if (strcmp(string1,"Symmetron") == 0 || strcmp(string1,"symmetron") == 0) {
        pba->gravity_model_smg = symmetron;
        pba->field_evolution_smg = _TRUE_;
        flag2=_TRUE_;
...
if (strcmp(string1,"nkgb") == 0 || strcmp(string1,"n-kgb") == 0 || strcmp(string1,"N-KGB") == 0 || strcmp(string1,"nKGB") == 0) {
  ...
  class_test(pba->parameters_smg[2]<0.,errmsg,"In n-KGB, Rshift0>=0, or ICs for background can't be set.");
      ...
      class_test(flag2==_FALSE_,
		 errmsg,
		 "could not identify gravity_theory value, check that it is one of 'propto_omega', 'propto_scale', 'constant_alphas', 'eft_alphas_power_law', 'eft_gammas_power_law', 'eft_gammas_exponential', 'brans_dicke', 'symmetron', 'galileon', 'nKGB', 'quintessence_monomial', 'quintessence_tracker', 'alpha_attractor_canonical' ...");
...
//     }
    ...
    if (flag1==_TRUE_){
      ...
      class_test(pba->gravity_model_smg!=brans_dicke && pba->gravity_model_smg!=symmetron,
		 errmsg,
		 "You asked to tune M_pl(today) to %e but currently this is only allowed for Brans-Dicke or the symmetron\n",
		 pba->M_pl_today_smg);
